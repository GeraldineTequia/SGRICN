 ...
 * consultar una necesidad específica.
 */
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const necesidad = await db.collection<Necesidad>("Necesidades").findOne({
      _id: id,
    });
    ...
    if (!necesidad) {
      return NextResponse.json(
        {
          success: false,
          message: "La necesidad no existe.",
        },
        { status: 404 }
      );
    }
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
     ...
     */
    const necesidadActual = await db
    ...
    await db.collection<Necesidad>("Necesidades").updateOne(
      {
        _id: id,
      },
      {
    ...
    const necesidadActualizada = await db
      .collection<Necesidad>("Necesidades")
      .findOne({
        _id: id,
      });
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const resultado = await db.collection<Necesidad>("Necesidades").deleteOne({
      _id: id,
    });
